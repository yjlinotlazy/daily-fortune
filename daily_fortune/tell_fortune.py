...
class FortuneTeller:
    ...
    def _read_db(self) -> pd.DataFrame:
        ...
        db["tags"] = db["tags_raw"].apply(
            lambda x: str(x).split("|") if not pd.isna(x) else []
        )
        return db[["text", "tags", "charlen"]]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="A simple fortune cookie util", epilog=epi
    )
    parser.add_argument("mode", nargs="?", type=str, help="random, seasonal, or hybrid")
    parser.add_argument(
        "--maxlen", nargs="?", type=str, help="max number of characters"
    )
